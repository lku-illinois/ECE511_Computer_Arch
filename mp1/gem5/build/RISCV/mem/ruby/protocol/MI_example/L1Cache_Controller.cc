...
L1Cache_Controller::init()
{
    ...
    m_cacheMemory_ptr->setRubySystem(m_ruby_system);
...
L1Cache_Controller::a_issueRequest(L1Cache_TBE*& m_tbe_ptr, L1Cache_Entry*& m_cache_entry_ptr, Addr addr)
{
    ...
    ((*m_requestFromCache_ptr)).enqueue(out_msg, clockEdge(), cyclesToTicks(Cycles(m_issue_latency)), m_ruby_system->getRandomization(), m_ruby_system->getWarmupEnabled());
}
    ...
    }
{
    ...
    ((*m_responseFromCache_ptr)).enqueue(out_msg, clockEdge(), cyclesToTicks(Cycles(m_cache_response_latency)), m_ruby_system->getRandomization(), m_ruby_system->getWarmupEnabled());
}
}
