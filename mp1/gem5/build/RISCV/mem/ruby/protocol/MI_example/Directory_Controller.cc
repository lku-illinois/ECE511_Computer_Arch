    ...
    : AbstractController(p)
{
    ...
    p.ruby_system->registerAbstractController(this, std::make_unique<MI_exampleProtocolInfo>());
...
Directory_Controller::init()
{
    ...
    possibleTransition(Directory_State_M_DWR, Directory_Event_PUTX_NotOwner);
    possibleTransition(Directory_State_M_DRD, Directory_Event_PUTX_NotOwner);
    possibleTransition(Directory_State_M_DRDI, Directory_Event_PUTX_NotOwner);
    possibleTransition(Directory_State_M_DWRI, Directory_Event_PUTX_NotOwner);
    ...
    }
{
    ...
    ((*m_forwardFromDir_ptr)).enqueue(out_msg, clockEdge(), cyclesToTicks(Cycles(m_directory_latency)), m_ruby_system->getRandomization(), m_ruby_system->getWarmupEnabled());
}
}
    ...
    }
{
    ...
    (*out_msg).m_MessageSize = MessageSizeType_Writeback_Control;
    ((*m_forwardFromDir_ptr)).enqueue(out_msg, clockEdge(), cyclesToTicks(Cycles((1))), m_ruby_system->getRandomization(), m_ruby_system->getWarmupEnabled());
}
}
    ...
    }
{
    ...
    ((*m_responseFromDir_ptr)).enqueue(out_msg, clockEdge(), cyclesToTicks(Cycles((1))), m_ruby_system->getRandomization(), m_ruby_system->getWarmupEnabled());
}
}
...
{
    std::shared_ptr<DMAResponseMsg> out_msg = std::make_shared<DMAResponseMsg>(clockEdge(),     m_ruby_system->getBlockSizeBytes(), m_ruby_system);
    #ifndef NDEBUG
    if (!((m_tbe_ptr != NULL))) {
    ...
    (*out_msg).m_MessageSize = MessageSizeType_Response_Data;
    ((*m_dmaResponseFromDir_ptr)).enqueue(out_msg, clockEdge(), cyclesToTicks(Cycles((1))), m_ruby_system->getRandomization(), m_ruby_system->getWarmupEnabled());
}
}
