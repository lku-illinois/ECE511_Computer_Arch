    ...
    : AbstractController(p)
{
    ...
    p.ruby_system->m_num_controllers[MachineType_DMA]++;
...
DMA_Controller::regStats()
{
    ...
    if (m_version == 0) {
        ...
        for (DMA_Event event = DMA_Event_FIRST;
            ...
            int num_controllers =
                m_ruby_system->m_num_controllers[MachineType_DMA];
...
{
    std::shared_ptr<DMARequestMsg> out_msg = std::make_shared<DMARequestMsg>(clockEdge(),     m_ruby_system->getBlockSizeBytes(), m_ruby_system);
    ...
    ((*m_requestToDir_ptr)).enqueue(out_msg, clockEdge(), cyclesToTicks(Cycles(m_request_latency)), m_ruby_system->getRandomization(), m_ruby_system->getWarmupEnabled());
}
}
